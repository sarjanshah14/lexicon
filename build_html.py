...
th,td{
  ...
  overflow-wrap:break-word;   /* wrap at spaces; break a long word only if it can't fit */
}
/* WIDE tables (6+ columns) are locked to the container width and wrap their text,
   so they shrink to fit when the sidebar is open instead of overflowing the page. */
table.wide{table-layout:fixed;}
table.wide th, table.wide td{overflow-wrap:break-word; word-break:break-word;}
/* glossary column proportions (8 cols: Word POS Meaning Root Tags Hook Syn Ant) */
table.gloss th:nth-child(1), table.gloss td:nth-child(1){width:11%;}
table.gloss th:nth-child(2), table.gloss td:nth-child(2){width:5%;}
table.gloss th:nth-child(3), table.gloss td:nth-child(3){width:18%;}
table.gloss th:nth-child(4), table.gloss td:nth-child(4){width:15%;}
table.gloss th:nth-child(5), table.gloss td:nth-child(5){width:6%;}
table.gloss th:nth-child(6), table.gloss td:nth-child(6){width:17%;}
...
// ===== mark wide tables (6+ columns) so they use a fixed, container-locked layout
// that wraps text and never overflows the page =====
document.querySelectorAll('table').forEach(t=>{
  const h=t.tHead||t.querySelector('thead');
  if(h && h.rows[0] && h.rows[0].cells.length>=6){
    t.classList.add('wide');
    // a leading "#" column should stay narrow
    const f=h.rows[0].cells[0];
    if(f && f.textContent.trim()==='#') f.style.width='2.5em';
  }
});
